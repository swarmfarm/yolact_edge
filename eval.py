...
def evaluate(net:Yolact, dataset, train_mode=False, train_cfg=None):
    ...
    try:
        if dataset.name == "YouTube VIS":
            # eval loop for YoutubeVIS
            # from torch.utils.tensorboard import SummaryWriter
            # wt = SummaryWriter(log_dir='../../logs/yolact_flow_eval/development')
            timer.enable_all()
                ...
                for pass_idx in range(num_passes):
                    ...
                    for frame_seq_idx, (extra, (img, gt, gt_masks, h, w, num_crowd)) in enumerate(zip(extra_data, video_frames)):
                        ...
                        if train_cfg.flow is not None:
                            if not train_cfg.dataset.use_all_frames or frame_idx % frame_eval_stride == pass_idx or not meet_annot or \
                                    (train_cfg.flow.warp_mode == 'none' and not train_cfg.flow.use_spa):
                                if frame_idx % frame_eval_stride == pass_idx: meet_annot = True
